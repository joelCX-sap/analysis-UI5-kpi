...
def analyze_kpi_with_context(kpi_query: str) -> str:
    """
    Specialized function for KPI analysis with enhanced context
    This function is specifically designed for the dashboard KPI analysis feature
    """
    
    print(f"🔍 Analyzing KPI with context: {kpi_query}")
    
    try:
        # Get current KPI data from HANA
        data_result, live_data = data_agent.query_live_data(kpi_query)
        
        if live_data is not None and not live_data.empty:
            # Enhanced KPI analysis prompt
            kpi_analysis_prompt = f"""
You are a specialized KPI Business Intelligence Analyst with access to live SAP HANA Purchase Documents data.

CURRENT DATA CONTEXT:
{data_result}

KPI ANALYSIS REQUEST: {kpi_query}

Based on the real-time data above, provide a comprehensive KPI analysis in Spanish that includes:

🎯 ANÁLISIS DEL KPI:
- Respuesta directa a la consulta basada en datos actuales
- Métricas clave y estadísticas relevantes
- Comparaciones y benchmarks cuando sea posible

📊 INSIGHTS PRINCIPALES:
- Patrones identificados en los datos
- Tendencias o anomalías observadas
- Factores que influyen en el rendimiento del KPI

💡 IMPACTO EMPRESARIAL:
- Qué significan estos resultados para el negocio
...
- KPIs adicionales a monitorear

NOTA IMPORTANTE: Este análisis está basado en {len(live_data):,} registros actuales extraídos en tiempo real de la base de datos HANA.

Responde en español con un tone profesional y analítico apropiado para ejecutivos de negocios.
"""
            
            # Get specialized KPI analysis
            llm = ChatOpenAI(proxy_model_name="gpt-4.1", proxy_client=proxy_client)
            analysis = llm.invoke(kpi_analysis_prompt).content
            
            print(f"✅ KPI Analysis completed for {len(live_data):,} live records")
            return analysis
            
        else:
            # Handle error case with Spanish response
            error_prompt = f"""
Hubo un problema al consultar la base de datos HANA para el análisis del KPI: "{kpi_query}"

Detalles del error: {data_result}

Por favor proporciona una respuesta útil en español explicando que hubo un problema técnico al acceder a los datos en tiempo real, y sugiere enfoques alternativos o recomienda verificar la conexión a la base de datos.

Responde en español con un tono profesional.
"""
        ...
        return f"Disculpa, hubo un error al acceder a los datos HANA en tiempo real: {str(e)}. Por favor verifica la conexión a la base de datos e intenta nuevamente."
